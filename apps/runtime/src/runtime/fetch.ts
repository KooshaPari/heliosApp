...
export async function handleRuntimeFetch(
	...
	if (
		url.pathname === "/v1/protocol/dispatch" &&
		...
		if (result.type !== "response") {
			return Response.json(
				{ error: "invalid_boundary_response" },
				{ status: 500 },
			);
		}
		...
		if (result.status === "error") {
			const status =
				result.error?.code === "UNSUPPORTED_BOUNDARY_ADAPTER" ? 409 : 400;
			return Response.json(
				{
	...
	if (
		url.pathname.match(/\/v1\/workspaces\/[^/]+\/lanes$/) &&
		requestInput.method === "POST"
	) {
	...
	if (
		...
		if (
			...
			return Response.json(
				{ error: "invalid_preferred_transport" },
				{ status: 400 },
			);
		}
		...
		return Response.json(
			{
				session_id: sessionId,
				transport:
					body.provider === "codex" ? "native_openai" : "cliproxy_harness",
				status: "attached",
