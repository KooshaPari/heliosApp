...
function classifyBootstrap(metadata: RecoveryMetadata): BootstrapResult {
  const recoveredSessionIds = metadata.sessions
  ...
  for (const lane of metadata.lanes) {
    if (
      lane.session_id &&
      !metadata.sessions.some(session => session.session_id === lane.session_id)
    ) {
      issues.push({
        id: lane.lane_id,
        state: "recoverable",
  ...
  for (const terminal of metadata.terminals) {
    if (
      terminal.session_id &&
      !metadata.sessions.some(session => session.session_id === terminal.session_id)
    ) {
      issues.push({
...
export function createRuntime(options: RuntimeOptions = {}): RuntimeHandle {
  ...
  let bootstrapResult = options.recovery_metadata
    ? classifyBootstrap(options.recovery_metadata)
    : null;
  ...
  async function spawnTerminal(
    input: RuntimeHandle["spawnTerminal"] extends (arg: infer T) => Promise<unknown> ? T : never
  ): Promise<LocalBusEnvelope> {
    ...
    if (response.status === "ok") {
      const terminalId = String(response.result?.terminal_id ?? `${input.session_id}:terminal`);
  ...
  async function inputTerminal(
    input: RuntimeHandle["inputTerminal"] extends (arg: infer T) => Promise<unknown> ? T : never
  ): Promise<LocalBusEnvelope> {
    ...
    if (
      ...
      return {
        ...
        error: {
          ...
          message: "terminal context mismatch",
          retryable: false,
        },
      };
    }
  ...
  async function resizeTerminal(
    input: RuntimeHandle["resizeTerminal"] extends (arg: infer T) => Promise<unknown> ? T : never
  ): Promise<LocalBusEnvelope> {
    const terminal = terminals.get(input.terminal_id);
    if (!terminal) {
      return {
  ...
  async function fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "POST" && /^\/v1\/workspaces\/[^/]+\/lanes$/.test(url.pathname)) {
      ...
      const laneId = String(body.display_name ?? body.project_context_id ?? `lane_${Date.now()}`);
      laneWorkspace.set(laneId, workspaceId);
      await bus.request({
        ...
        method: "lane.create",
        payload: { id: laneId, lane_id: laneId },
      });
      return json({ lane_id: laneId }, 201);
    }

    if (
      request.method === "POST" &&
      /\/v1\/workspaces\/[^/]+\/lanes\/[^/]+\/sessions$/.test(url.pathname)
    ) {
      const [, , , workspaceId, , laneId] = url.pathname.split("/");
      const preferredTransport = body.preferred_transport;
      if (
        preferredTransport !== undefined &&
        preferredTransport !== "cliproxy_harness" &&
        preferredTransport !== "native_openai"
      ) {
      ...
      const degradeReason = harness.ok ? null : (harness.reason ?? "cliproxy_unavailable");
      const sessionId = String(body.codex_session_id ?? `${laneId}:session`);
      await bus.request({
        ...
        payload: {
          ...
          lane_id: laneId,
          session_id: sessionId,
          ...(degradeReason ? { boundary_failure: "harness" } : {}),
        },
      });
      if (!harness.ok) {
        await bus.publish({
          ...
          workspace_id: workspaceId,
          lane_id: laneId,
          payload: { status: "unavailable", degrade_reason: degradeReason },
        });
      }
      return json({
        session_id: sessionId,
        ...
        transport,
        status: "attached",
        diagnostics: { degrade_reason: degradeReason },
      });
    }

    if (
      request.method === "POST" &&
      /\/v1\/workspaces\/[^/]+\/lanes\/[^/]+\/terminals$/.test(url.pathname)
    ) {
      const [, , , workspaceId, , laneId] = url.pathname.split("/");
      if (laneWorkspace.get(laneId) && laneWorkspace.get(laneId) !== workspaceId) {
        return json({ error: "lane does not belong to workspace" }, 409);
      }
      if (closedLanes.has(laneId)) {
        return json({ error: "lane_closed" }, 409);
      }
      const sessionId = String(body.session_id ?? `${laneId}:session`);
    ...
    if (
      request.method === "POST" &&
      /\/v1\/workspaces\/[^/]+\/lanes\/[^/]+\/cleanup$/.test(url.pathname)
    ) {
      const laneId = url.pathname.split("/")[5] ?? "";
      closedLanes.add(laneId);
      return json({ ok: true });
    }
  ...
  function exportRecoveryMetadata(): RecoveryMetadata {
    return {
      lanes: [...laneWorkspace.entries()].map(([laneId, workspaceId]) => ({
        lane_id: laneId,
        workspace_id: workspaceId,
      })),
