...
type RuntimeAuditRecord = Awaited<
	ReturnType<InMemoryLocalBus["getAuditRecords"]>
>[number] & {
	recorded_at: string;
};
...
function classifyBootstrap(metadata: RecoveryMetadata): BootstrapResult {
	...
	for (const lane of metadata.lanes) {
		if (
			lane.session_id &&
			!metadata.sessions.some(
				(session) => session.session_id === lane.session_id,
			)
		) {
			issues.push({
				id: lane.lane_id,
	...
	for (const terminal of metadata.terminals) {
		if (
			terminal.session_id &&
			!metadata.sessions.some(
				(session) => session.session_id === terminal.session_id,
			)
		) {
			issues.push({
...
export function createRuntime(options: RuntimeOptions = {}): RuntimeHandle {
	...
	async function spawnTerminal(
		input: RuntimeHandle["spawnTerminal"] extends (
			arg: infer T,
		) => Promise<unknown>
			? T
			: never,
		...
		if (response.status === "ok") {
			const terminal_id = String(
				response.result?.terminal_id ?? `${input.session_id}:terminal`,
			);
			...
			terminalBuffers.set(terminal_id, {
				entries: [],
				total_bytes: 0,
				dropped_bytes: 0,
				next_seq: 1,
	...
	async function inputTerminal(
		input: RuntimeHandle["inputTerminal"] extends (
			arg: infer T,
		) => Promise<unknown>
			? T
			: never,
	...
	async function resizeTerminal(
		input: RuntimeHandle["resizeTerminal"] extends (
			arg: infer T,
		) => Promise<unknown>
			? T
			: never,
	...
	async function fetch(request: Request): Promise<Response> {
		...
		if (request.method === "POST" && url.pathname === "/v1/protocol/dispatch") {
			const response = await dispatcher({
				...
				terminal_id:
					typeof body.terminal_id === "string" ? body.terminal_id : undefined,
		...
		if (
			request.method === "POST" &&
			/^\/v1\/workspaces\/[^/]+\/lanes$/.test(url.pathname)
		) {
			const workspace_id = url.pathname.split("/")[3] ?? "ws_1";
			const lane_id = String(
				body.display_name ?? body.project_context_id ?? `lane_${Date.now()}`,
			);
		...
		if (
			...
			if (
				...
				preferred_transport !== "native_openai"
			) {
			...
			const degrade_reason = harness.ok
				? null
				: (harness.reason ?? "cliproxy_unavailable");
		...
		if (
			...
			const [, , , workspace_id, , lane_id] = url.pathname.split("/");
			if (
				laneWorkspace.get(lane_id) &&
				laneWorkspace.get(lane_id) !== workspace_id
			) {
		...
		if (
			request.method === "GET" &&
			url.pathname === "/v1/harness/cliproxy/status"
		) {
			const harness = (await options.harnessProbe?.check?.()) ?? { ok: true };
			return json({
				status: harness.ok ? "healthy" : "unavailable",
				degrade_reason: harness.ok
					? null
					: (harness.reason ?? "cliproxy_unavailable"),
			});
		}
	...
	function exportRecoveryMetadata(): RecoveryMetadata {
		return {
			lanes: [...laneWorkspace.entries()].map(([lane_id, workspace_id]) => ({
				lane_id,
				workspace_id,
			})),
		...
		getState() {
			...
			if (
				[...terminals.values()].some(
					(terminal) => terminal.state === "throttled",
				)
			) {
