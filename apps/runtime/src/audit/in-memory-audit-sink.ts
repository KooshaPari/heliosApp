...
export class InMemoryAuditSink implements AuditSink {
	...
	private flattenRecord(
		...
		const envelope = redactPayload
			? this.redactEnvelope(record.envelope)
			: { ...record.envelope };
		const methodOrTopic = (envelope.method ?? envelope.topic) as
			| string
			| undefined;
		...
		return {
			...
			workspace_id:
				typeof envelope.workspace_id === "string"
					...
					: undefined,
			lane_id:
				typeof envelope.lane_id === "string" ? envelope.lane_id : undefined,
			session_id:
				typeof envelope.session_id === "string"
					? envelope.session_id
					: undefined,
			terminal_id:
				typeof envelope.terminal_id === "string"
		...
		const entries = Object.entries(value as Record<string, unknown>).map(
			([entryKey, entryValue]) => {
				if (
					...
					this.shouldRedact(entryKey, entryValue)
				) {
