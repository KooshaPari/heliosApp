...
export class AuditLedger {
	...
	private batchedNotifications: Map<SubscriptionCallback, AuditEvent[]> =
		new Map();
	...
	private traverseCorrelationChain(
		...
		if (events.length === 0) {
			console.warn(
				`[AuditLedger] No events found for correlation ID: ${correlationId}`,
			);
			return;
		}
	...
	private filterToRingBufferFilter(filter: AuditFilter): RingBufferFilter {
		...
		if (filter.eventType) {
			rbFilter.eventType = Array.isArray(filter.eventType)
				? filter.eventType[0]
				: filter.eventType;
		}
	...
	private matchesFilter(event: AuditEvent, filter: AuditFilter): boolean {
		...
		if (filter.timeRange) {
			...
			if (
				eventTime < filter.timeRange.from ||
				eventTime > filter.timeRange.to
			) {
				return false;
			}
		}
