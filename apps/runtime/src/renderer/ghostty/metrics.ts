...
export class GhosttyMetrics {
	...
	constructor(config: MetricsConfig = {}) {
		...
		this._publishIntervalMs =
			config.publishIntervalMs ?? DEFAULT_PUBLISH_INTERVAL_MS;
	...
	getSnapshot(): MetricsSnapshot {
		...
		return {
			...
			p50InputLatency:
				Math.round(percentile(sortedInputLatencies, 50) * 100) / 100,
			p95InputLatency:
				Math.round(percentile(sortedInputLatencies, 95) * 100) / 100,
			timestamp: now,
			rendererId: "ghostty",
		};
	}
