...
export async function handleRioToggle(
	...
	if (!newEnabled) {
		...
		if (existing) {
			...
			if (!available) {
				console.warn(
					"Rio renderer: feature flag enabled but rio binary not found",
				);
			...
			try {
				...
				console.error(
					"Rio renderer: failed to load backend module during toggle",
				);
				events.push({ type: "renderer.rio.enabled" });
				return events;
			}
...
export class RioToggleQueue {
	...
	private async _processQueue(): Promise<void> {
		...
		while (this._pending.length > 0) {
			...
			const item =
				this._pending.length > 1 ? this._drainToLast() : this._pending.shift()!;

			try {
				const events = await handleRioToggle(
					this._registry,
					item.enabled,
					this._config,
				);
