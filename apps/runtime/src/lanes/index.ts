...
export class LaneManager {
	...
	async detach(laneId: string, agentId: string): Promise<void> {
		...
		if (
			result.transitioned &&
			result.fromState !== undefined &&
			result.toState !== undefined
		) {
			const lane = this.registry.get(laneId);
			await this.emitEvent(
				"lane.state.changed",
	...
	private async terminateLanePtys(
		laneId: string,
		workspaceId: string,
	): Promise<void> {
		...
		const terminationPromises = ptys.map(async (pty) => {
			try {
				...
				const termination = this.ptyManager!.terminate(pty.ptyId).then(
					() => "done" as const,
				);
	...
	private async emitReconciliationEvent(
