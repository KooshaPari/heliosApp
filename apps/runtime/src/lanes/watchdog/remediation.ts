...
export class RemediationEngine {
		...
		for (const orphan of orphans) {
			...
			if (orphan.estimatedOwner !== "unknown") {
				try {
					...
					if (
						lane &&
						(lane.state === "cleaning" || lane.state === "provisioning")
					) {
	...
	private async cleanupPtyProcess(
		orphan: ClassifiedOrphan,
	): Promise<CleanupResult> {
		if (!orphan.pid) {
			return {
		...
		try {
			// Send SIGTERM
			const killResult = await execCommand("kill", [
				"-TERM",
				String(orphan.pid),
			]);
			...
			if (killResult.code === 0 || killResult.code === 1) {
				...
				const checkResult = await execCommand("kill", [
					"-0",
					String(orphan.pid),
				]);
				...
				const killResult2 = await execCommand("kill", [
					"-KILL",
					String(orphan.pid),
				]);
