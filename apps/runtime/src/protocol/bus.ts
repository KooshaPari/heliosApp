	...
	async request(command: LocalBusEnvelope): Promise<LocalBusEnvelope> {
		...
		if (command.method) {
			...
			if (command.method === "lane.create") {
				...
				const resolvedTransport = degraded
					? "native_openai"
					: preferredTransport;
...
class CommandBusImpl implements CommandBus {
		...
		if (this.destroyed) {
			const id =
				typeof (envelope as Record<string, unknown>)?.id === "string"
					...
					: "unknown";
			const corr =
				typeof (envelope as Record<string, unknown>)?.correlation_id ===
				"string"
	...
	async publish(evt: unknown): Promise<void> {
		...
		if (this.activeCorrelationId) {
			(event as Record<string, unknown>).correlation_id =
	...
	private resolveContext(command: CommandEnvelope): {
		...
		return {
			workspace_id:
				command.workspace_id ?? this.readString(payload.workspace_id),
