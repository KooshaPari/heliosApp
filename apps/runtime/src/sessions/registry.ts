		...
		if (activeSessionId) {
			...
			if (!active) {
				...
				if (
					...
					input.codex_session_id !== active.codex_session_id
				) {
...
export class RecoveryRegistry {
	...
	scanForOrphans(nowIso: string): WatchdogScanResult {
		...
		for (const session of this.sessions.values()) {
			const hasValidLane = Boolean(
				session.lane_id && this.lanes.has(session.lane_id),
			);
			if (!hasValidLane) {
