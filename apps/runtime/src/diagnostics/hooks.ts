...
interface HooksState {
	...
	onSample:
		| ((metric: string, value: number, timestamp: number) => void)
		| undefined;
}
...
let globalState: HooksState = createState(
	DEFAULT_MAX_CONCURRENT_MARKS,
	defaultClock,
);
...
export function createInstrumentationHooks(opts?: {
	...
	return {
		markStart(metric: string): number {
			...
			if (
				state.totalMarks >= state.startTimes.length &&
				state.metricNames[slot] !== ""
			) {
				state.overflowCount++;
			}
		...
		markEnd(metric: string, handle: number): number {
			if (handle < 0 || handle >= state.startTimes.length) {
				console.warn(
					`[perf] markEnd called with out-of-range handle ${handle}`,
				);
				return Number.NaN;
			}
