...
export class RingBuffer {
	...
	getValues(): Float64Array {
		...
		result.set(
			this.values.subarray(this.writeIndex, this.writeIndex + tailLen),
			0,
		);
...
export class MetricsRegistry {
	...
	record(name: string, value: number, timestamp?: number): void {
		...
		if (entry === undefined) {
			console.warn(
				`[metrics] Attempted to record to unregistered metric "${name}"`,
			);
			return;
		}
		// Lazy buffer allocation.
		if (entry.buffer === undefined) {
			entry.buffer = new RingBuffer(
				entry.definition.bufferSize ?? DEFAULT_CAPACITY,
			);
		}
