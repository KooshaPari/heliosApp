...
export function getDefaultRules(): RedactionRule[] {
	return [
		...
		},
		{
			...
			category: "API_KEY",
			pattern:
				/(?:api_key|apikey|api_token)\s*[=:]\s*["']?([A-Za-z0-9\-_]{16,})["']?/i,
...
export class RedactionRuleManager {
	private rules: Map<string, RedactionRule & { matchCount: number }> =
		new Map();
	...
	listRules(): RedactionRule[] {
		return Array.from(this.rules.values()).map(
			({ matchCount: _mc, ...rule }) => rule,
		);
	}
	...
	exportRules(filePath: string): void {
		const data = Array.from(this.rules.values()).map(
			({ matchCount: _mc, pattern, ...rest }) => ({
				...rest,
