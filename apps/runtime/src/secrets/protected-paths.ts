...
const DEFAULT_PATTERNS: ProtectedPathPattern[] = [
	{
		...
		pattern: ".env",
		description:
			".env files (exact and variants like .env.local, .env.production)",
		enabled: true,
		isDefault: true,
	},
	{
...
function matchesPattern(filePath: string, pattern: string): boolean {
	...
	if (!pattern.includes("*") && !pattern.includes("?")) {
		...
		if (
			expandedPath.endsWith(expandedPattern) ||
...
export class ProtectedPathConfig {
	...
	addPattern(pattern: string, description: string): ProtectedPathPattern {
		...
		if (
			...
			throw new Error(
				`Pattern '${pattern}' is too broad and would match all paths`,
			);
		}
			...
			for (const pattern of enabledPatterns) {
				if (matchesPattern(filePath, pattern.pattern)) {
					...
					void this._emit("secrets.protected_path.accessed", {
						...
						correlationId:
							opts?.correlationId ?? randomBytes(8).toString("hex"),
					});
