...
describe("Default rules: positive examples", () => {
	...
	it("Connection string postgres - positive", () => {
		...
		expect(r.matches.some((m) => m.category === "CONNECTION_STRING")).toBe(
			true,
		);
	});
...
describe("Default rules: negative examples", () => {
	...
	it("AWS Access Key - negative (too short)", () => {
		...
		expect(
			r.matches.filter((m) => m.category === "AWS_ACCESS_KEY").length,
		).toBe(0);
	});
	...
	it("GCP key - negative (wrong prefix)", () => {
		...
		expect(r.matches.filter((m) => m.category === "GCP_API_KEY").length).toBe(
			0,
		);
	});
	...
	it("Private key - negative (wrong header)", () => {
		...
		expect(r.matches.filter((m) => m.category === "PRIVATE_KEY").length).toBe(
			0,
		);
	});
});
...
describe("RedactionRuleManager: enable/disable", () => {
	...
	it("removeRule removes the rule", () => {
		...
		expect(manager.listRules().some((r) => r.id === "aws-access-key")).toBe(
			false,
		);
	});
});
...
describe("RedactionRuleManager: bus events", () => {
	it("emits event on rule change", async () => {
		...
		expect(
			events.some((e) => e.topic === "secrets.redaction.rules.changed"),
		).toBe(true);
	});
});
