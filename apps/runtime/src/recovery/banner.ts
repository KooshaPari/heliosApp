...
export class RecoveryBanner {
	...
	show(stage: RecoveryStage): void {
		...
		this.isActive =
			stage !== RecoveryStage.LIVE && stage !== RecoveryStage.CRASHED;
		this.renderBanner();
	}
	...
	private getStageMessage(stage: RecoveryStage): string {
		const messages: Record<RecoveryStage, string> = {
			...
			[RecoveryStage.DETECTION_FAILED]:
				"Crash detection failed. Manual recovery may be needed.",
			[RecoveryStage.INVENTORY_FAILED]:
				"Failed to inventory state. Some sessions may be lost.",
			[RecoveryStage.RESTORATION_FAILED]:
				"Session restoration encountered errors.",
			[RecoveryStage.RECONCILIATION_FAILED]:
				"Orphan reconciliation encountered errors.",
		};
