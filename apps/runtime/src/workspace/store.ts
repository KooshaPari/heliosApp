	...
	async acquire(): Promise<() => void> {
		return new Promise<() => void>((resolve, reject) => {
			const timer = setTimeout(() => {
				...
				reject(
					new Error(
						"Write lock timeout: could not acquire lock within 5 seconds",
					),
				);
			}, LOCK_TIMEOUT_MS);
...
export class JsonWorkspaceStore implements WorkspaceStore {
	...
	async load(): Promise<void> {
		...
		if (typeof envelope._checksum === "string") {
			...
			if (envelope._checksum !== expected) {
				console.warn(
					"[workspace-store] Checksum mismatch in primary file, attempting recovery",
				);
				await this.attemptRecovery();
				return;
			}
	...
	private async attemptRecovery(): Promise<void> {
		...
		if (corruption.corrupted) {
			console.warn(
				`[workspace-store] Primary file corrupted: ${corruption.reason}`,
			);
		}
...
// FR-005
export async function createJsonStore(
	dataDir: string,
