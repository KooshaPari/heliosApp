...
export type RulesChangedCallback = (
	workspaceId: string,
	rules: PolicyRule[],
) => void;
...
export class PolicyStorage {
	...
	async loadRules(workspaceId: string): Promise<PolicyRule[]> {
		...
		try {
			...
			throw new Error(
				`Failed to load policy rules for ${workspaceId}: ${error}`,
			);
		}
	}
			...
			try {
				...
				if (mtime > lastMtime && lastMtime > 0) {
					...
					try {
						...
						console.error(
							`Failed to reload policy rules for ${workspaceId}:`,
							error,
						);
	 ...
	 */
	private notifyChangedDebounced(
		workspaceId: string,
		rules: PolicyRule[],
	): void {
