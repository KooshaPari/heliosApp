	...
	return {
		...
		run: mock(async (args: string[]) => {
			if (args[0] === "kill-session") {
				const name = args[1]!;
				return (
					killResults?.get(name) ?? { stdout: "", stderr: "", exitCode: 0 }
				);
			}
...
describe("reconcile", () => {
	it("terminates orphaned helios sessions", async () => {
		...
		registry.bind(
			"helios-lane-bound",
			"bound",
			makeMuxSession("helios-lane-bound", "bound"),
		);
	...
	it("cleans up stale registry entries for dead sessions", async () => {
		...
		registry.bind(
			"helios-lane-dead",
			"dead",
			makeMuxSession("helios-lane-dead", "dead"),
		);
	...
	it("handles mixed orphans and stale bindings", async () => {
		...
		registry.bind(
			"helios-lane-alive",
			"alive",
			makeMuxSession("helios-lane-alive", "alive"),
		);
		registry.bind(
			"helios-lane-gone",
			"gone",
			makeMuxSession("helios-lane-gone", "gone"),
		);
