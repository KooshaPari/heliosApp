...
describe("Integration: full lifecycle", () => {
	...
	beforeEach(() => {
		...
		sessionMgr = new ZellijSessionManager(
			cli as unknown as ZellijCli,
			registry,
			{
				topology,
				emitter,
	...
	it("create session, add tabs, switch, close tab", async () => {
		...
		expect(topology.getTopology(session.sessionName)?.activeTabId).toBe(
			tab.index,
		);
...
describe("Integration: reattach", () => {
	it("creates session, simulates restart, reattaches, verifies topology", async () => {
		...
		const sessionMgr = new ZellijSessionManager(
			cli as unknown as ZellijCli,
			registry,
			{
				topology,
