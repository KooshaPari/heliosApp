...
async function ensureSession(
	...
	return runtime.fetch(
		jsonRequest(
			`http://localhost/v1/workspaces/ws_1/lanes/${laneId}/sessions`,
			body,
		),
	);
}
...
describe("session routing lifecycle", () => {
	it("uses cliproxy_harness when health check is healthy", async () => {
		...
		expect(
			events.some((event) => event.topic === "session.created"),
		).toBeTrue();
	});
	...
	it("falls back to native_openai with explicit degrade reason when harness is unavailable", async () => {
		...
		const events = runtime.getEvents();
		expect(
			events.some((event) => event.topic === "harness.status.changed"),
		).toBeTrue();
	});
	...
	it("implements terminal spawn endpoint with protocol lifecycle events", async () => {
		...
		const terminalResponse = await runtime.fetch(
			jsonRequest(
				`http://localhost/v1/workspaces/ws_1/lanes/${laneId}/terminals`,
				{
		...
		const events = runtime.getEvents();
		expect(
			events.some((event) => event.topic === "terminal.spawn.started"),
		).toBeTrue();
		expect(
			events.some((event) => event.topic === "terminal.state.changed"),
		).toBeTrue();
		expect(
			events.some((event) => event.topic === "terminal.spawned"),
		).toBeTrue();
	...
	it("rejects terminal spawn when lane workspace does not match route workspace", async () => {
		...
		const terminalResponse = await runtime.fetch(
			jsonRequest(
				`http://localhost/v1/workspaces/ws_2/lanes/${laneId}/terminals`,
				{
					session_id: sessionBody.session_id,
					title: "Spoofed",
				},
	...
	it("rejects terminal spawn when lane is closed", async () => {
		...
		const cleanupResponse = await runtime.fetch(
			jsonRequest(
				`http://localhost/v1/workspaces/ws_1/lanes/${laneId}/cleanup`,
				{},
			),
		);
