...
describe("Event ordering — per-topic monotonic sequences", () => {
	...
	it("maintains independent sequence counters per topic", async () => {
		...
		for (let i = 0; i < 100; i++) {
			promises.push(
				bus.publish(createEvent("topic.a", { i }) as LocalBusEnvelope),
			);
			promises.push(
				bus.publish(createEvent("topic.b", { i }) as LocalBusEnvelope),
			);
		}
		await Promise.all(promises);
	...
	it("handles 10 topics concurrently with independent monotonic sequences", async () => {
		...
		for (let i = 0; i < 100; i++) {
			for (const topic of topicNames) {
				promises.push(
					bus.publish(createEvent(topic, { i }) as LocalBusEnvelope),
				);
			}
		}
		await Promise.all(promises);
