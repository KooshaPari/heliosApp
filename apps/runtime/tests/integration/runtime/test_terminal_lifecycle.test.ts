...
describe("terminal lifecycle and streaming data plane", () => {
	...
	test("spawns terminals, preserves correlation, and blocks cross-lane access", async () => {
		...
		const spawnOneEvents = events.filter(
			(event) => event.correlation_id === "corr-spawn-1",
		);
		...
		expect(
			spawnOneEvents.every((event) => event.correlation_id === "corr-spawn-1"),
		).toBe(true);
		...
		const firstEnvelope = (auditRecords[0]?.envelope ?? {}) as Record<
			string,
			unknown
		>;
	...
	test("clears stale buffered output when reusing terminal_id", async () => {
		...
		expect(
			runtime
				.getTerminalBuffer("term-reused")
				.entries.map((entry: { seq: number }) => entry.seq),
		).toEqual([1]);
