...
export class BaseMockAdapter implements RendererAdapter {
	...
	async init(_config: RendererConfig): Promise<void> {
		...
		if (
			this._opts.initFailAfter !== undefined &&
			this._initCallCount > this._opts.initFailAfter
		) {
			this._state = "errored";
			throw new Error(
				`${this.id} init failed (after ${this._opts.initFailAfter} calls)`,
			);
		}
...
export function createMockPtyStream(
	chunks: Uint8Array[],
