...
describe("TerminalRegistry", () => {
  ...
  test("enforces ownership boundaries for workspace lane session", () => {
    ...
    expect(registry.isOwnedBy("t-2", { workspaceId: "ws-1", laneId: "lane-1", sessionId: "sess-1" })).toBe(true);
    expect(registry.isOwnedBy("t-2", { workspaceId: "ws-1", laneId: "lane-2", sessionId: "sess-1" })).toBe(false);
  });

  test("cleans up session scoped terminals", () => {
    const registry = new TerminalRegistry();
    registry.spawn({ terminalId: "t-3", workspaceId: "ws-1", laneId: "lane-1", sessionId: "sess-1" });
    registry.spawn({ terminalId: "t-4", workspaceId: "ws-1", laneId: "lane-1", sessionId: "sess-1" });
  ...
  test("re-indexes terminal ownership when terminal_id is reused", () => {
    const registry = new TerminalRegistry();
    registry.spawn({ terminalId: "t-5", workspaceId: "ws-1", laneId: "lane-1", sessionId: "sess-1" });
    registry.spawn({ terminalId: "t-5", workspaceId: "ws-1", laneId: "lane-2", sessionId: "sess-2" });
