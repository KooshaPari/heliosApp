...
describe("protocol validator", () => {
	test("rejects malformed envelope with stable error semantics", () => {
		expect(() =>
			validateEnvelope({ id: "evt-1", ts: "2026-02-26T00:00:00.000Z" }),
		).toThrow(ProtocolValidationError);
		expect(() =>
			validateEnvelope({ id: "evt-1", ts: "2026-02-26T00:00:00.000Z" }),
		).toThrow("Envelope field 'type' is required");
	});
	...
	test("rejects optional timestamp without RFC3339 timezone", () => {
		expect(() =>
			...
			"Envelope field 'timestamp' must be an RFC3339 timestamp with timezone",
		);
	});
