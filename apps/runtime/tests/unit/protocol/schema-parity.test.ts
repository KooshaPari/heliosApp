...
function validateSchema(
	...
	if (schema.oneOf !== undefined) {
		const matches = schema.oneOf.filter(
			(sub) => validateSchema(data, sub, path).length === 0,
		);
		if (matches.length === 0) {
...
describe("JSON Schema parity — runtime envelopes match canonical schema", () => {
	...
	it("required fields in JSON schema match TypeScript type requirements", () => {
		...
		// oneOf branches
		const commandSchema = schema.oneOf?.find(
			(s) => s.title === "CommandEnvelope",
		);
		const responseSchema = schema.oneOf?.find(
			(s) => s.title === "ResponseEnvelope",
		);
