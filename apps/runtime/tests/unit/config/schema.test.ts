...
describe("validateValue", () => {
	...
	it("rejects number above range", () => {
		expect(validateValue("terminal.scrollback_lines", 200000).valid).toBe(
			false,
		);
	});
	...
	// NaN / Infinity
	it("rejects NaN", () => {
		expect(validateValue("terminal.scrollback_lines", Number.NaN).valid).toBe(
			false,
		);
	});
	...
	it("rejects Infinity", () => {
		expect(
			validateValue("terminal.scrollback_lines", Number.POSITIVE_INFINITY)
				.valid,
		).toBe(false);
	});
