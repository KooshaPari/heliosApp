...
describe("OutputBuffer", () => {
	...
	it("emits backpressure.on when crossing threshold", () => {
		...
		const onEvents = bus.events.filter(
			(e) => e.topic === "pty.backpressure.on",
		);
	...
	it("emits backpressure.off with hysteresis band", () => {
		...
		const offEvents = bus.events.filter(
			(e) => e.topic === "pty.backpressure.off",
		);
	...
	it("emits overflow event and logs warning on first overflow", () => {
		...
		try {
			...
			const overflowEvts = bus.events.filter(
				(e) => e.topic === "pty.buffer.overflow",
			);
