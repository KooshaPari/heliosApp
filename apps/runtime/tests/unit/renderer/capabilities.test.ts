...
describe("compareCapabilities", () => {
	...
	it("detects scalar differences", () => {
		...
		expect(diff.differences.some((d) => d.field === "gpuAccelerated")).toBe(
			true,
		);
		expect(diff.differences.some((d) => d.field === "colorDepth")).toBe(true);
		expect(diff.differences.some((d) => d.field === "ligatureSupport")).toBe(
			true,
		);
	});
	...
	it("detects maxDimensions differences", () => {
		const diff = compareCapabilities(CAPS_A, CAPS_B);
		expect(diff.differences.some((d) => d.field === "maxDimensions")).toBe(
			true,
		);
	});
	...
	it("compares ghostty vs rio capabilities", () => {
		...
		const diff = compareCapabilities(
			g.queryCapabilities(),
			r.queryCapabilities(),
		);
