...
describe("binding_triple", () => {
	...
	describe("validateBindingTriple", () => {
		...
		it("should reject invalid workspace ID format", () => {
			...
			expect(
				result.errors.some((e) => e.includes("Invalid workspace ID format")),
			).toBe(true);
		});
		...
		it("should reject invalid lane ID format", () => {
			...
			expect(result.valid).toBe(false);
			expect(
				result.errors.some((e) => e.includes("Invalid lane ID format")),
			).toBe(true);
		});
		...
		it("should reject invalid session ID format", () => {
			...
			expect(result.valid).toBe(false);
			expect(
				result.errors.some((e) => e.includes("Invalid session ID format")),
			).toBe(true);
		});
		...
		it("should reject when workspace does not exist", () => {
			...
			expect(result.valid).toBe(false);
			expect(
				result.errors.some((e) => e.includes("Workspace does not exist")),
			).toBe(true);
		});
		...
		it("should reject when session does not belong to lane", () => {
			...
			expect(result.valid).toBe(false);
			expect(
				result.errors.some((e) => e.includes("does not belong to lane")),
			).toBe(true);
		});
