...
import type { PtyHandle, PtyManager } from "../../src/lanes/index.js";
import {
...
describe("T006 - provisionWorktree", () => {
	...
	test("creates worktree directory and branch", async () => {
		...
		const branches = Bun.spawnSync(
			["git", "branch", "--list", result.branchName],
			{ cwd: repo },
		);
	...
	test("removes stale worktree path before provisioning", async () => {
		...
		expect(fs.existsSync(path.join(result.worktreePath, "stale.txt"))).toBe(
			false,
		);
...
describe("T009 - Orphan reconciliation", () => {
	...
	test("skips known lanes during reconciliation", async () => {
		...
		const reconcileResult = await reconcileOrphanedWorktrees(
			repo,
			knownLanes,
			() => {},
		);
