...
describe("TabBar", () => {
	...
	describe("Tab Reordering", () => {
		it("should return correct tab order", () => {
			expect(tabBar.getTabOrder()).toEqual([
				"tab1",
				"tab2",
				"tab3",
				"tab4",
	...
	describe("Tab Pinning", () => {
		...
		it("should place pinned tabs first in display order", () => {
			...
			const renderedOrder = Array.from(tabHeaders).map((el) =>
	...
	describe("Tab Rendering", () => {
		...
		it("should highlight selected tab", () => {
			...
			expect(selectedTab.style.backgroundColor).toMatch(
				/rgb\(255.*255.*255\)|#fff(fff)?|white/,
			);
		});
