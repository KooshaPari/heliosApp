...
test("lane/session context remains cohesive across all tabs", async ({
	...
	for (const tab of ["terminal", "agent", "session", "chat", "project"]) {
		await expect(page.getByTestId(`tab-${tab}-workspace`)).toHaveText(
			"workspace_e2e",
		);
		await expect(page.getByTestId(`tab-${tab}-lane`)).toHaveText(
			lane.laneId as string,
		);
		await expect(page.getByTestId(`tab-${tab}-session`)).toHaveText(
			session.sessionId as string,
		);
	}

	await expect(page.getByTestId("tab-chat-transport")).toHaveText(
		"cliproxy_harness",
	);
	await expect(page.getByTestId("tab-chat-degrade")).toHaveText("none");
});

test("renderer switch failure rolls back and reports safe status", async ({
	page,
}) => {
	const runtime = createRuntime();
	const controlPlane = bootDesktop({ bus: runtime.bus });
	...
	await expect(page.getByTestId("renderer-switch-status")).toHaveText(
		"rolled_back",
	);
});

test("lane lifecycle supports session restore after reconnect", async ({
	page,
}) => {
	const runtime = createRuntime();
	const controlPlane = bootDesktop({ bus: runtime.bus });
	...
	await expect(page.getByTestId("tab-session-workspace")).toHaveText(
		"workspace_restore",
	);
	await expect(page.getByTestId("tab-session-lane")).toHaveText(
		lane.laneId as string,
	);
	await expect(page.getByTestId("tab-session-session")).toHaveText(
		session.sessionId as string,
	);
