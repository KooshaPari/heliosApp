	...
	return {
		...
		resolvedTransport:
			typeof diagnostics.resolved_transport === "string"
				...
				: "cliproxy_harness",
		degradedReason:
			typeof diagnostics.degraded_reason === "string"
				? diagnostics.degraded_reason
				: null,
		degradedAt:
			typeof diagnostics.degraded_at === "string"
				? diagnostics.degraded_at
				: null,
	};
}
...
export class DesktopRuntimeClient {
	...
	async createLane(input: {
		...
		return {
			...
			id:
				typeof parsed.result?.lane_id === "string"
					? parsed.result.lane_id
					: null,
	...
	async spawnTerminal(input: {
		...
		return {
			...
			runtimeState: (parsed.result?.state as RuntimeState | undefined) ?? null,
			id:
				typeof parsed.result?.terminal_id === "string"
					? parsed.result.terminal_id
					: null,
			diagnostics: normalizeDiagnostics(parsed.result),
			error: parsed.error,
		};
	...
	async getRendererCapabilities(
		...
		const parsed = toResponse<Record<string, unknown>>(response);
		const activeEngine =
			parsed.result?.active_engine === "rio" ? "rio" : "ghostty";
		const available: RendererEngine[] = Array.isArray(
			parsed.result?.available_engines,
		)
			? parsed.result.available_engines.filter(
					(value): value is RendererEngine =>
						value === "ghostty" || value === "rio",
				)
			: ["ghostty", "rio"];
		return {
			activeEngine,
	...
	async switchRenderer(input: {
		...
		return {
			...
			previousEngine:
				parsed.result?.previous_engine === "rio" ? "rio" : "ghostty",
			error: parsed.error,
		};
	}
}
