...
export class RendererPreferencesManager {
	...
	constructor(preferencesPath?: string) {
		if (preferencesPath) {
			...
			this.preferencesPath = resolve(
				heliosDataDir,
				"renderer_preferences.json",
			);
		}
	}
		...
		try {
			if (this.doesFileExist()) {
				...
				if (this.isValidPreferences(loaded)) {
					...
					console.log(
						`Renderer preferences loaded in ${loadTime.toFixed(2)}ms`,
					);
			...
			console.warn(
				"Failed to load renderer preferences, using defaults",
				error,
			);
		}
