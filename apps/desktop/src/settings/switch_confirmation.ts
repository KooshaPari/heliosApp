...
export class SwitchConfirmation {
	...
	private createAndShowDialog(): void {
		...
		if (this.props.supportsHotSwap) {
			...
			method.textContent =
				"This will restart the renderer with session restore (~8 seconds).";
...
export function createSwitchConfirmation(
	props: SwitchConfirmationProps,
): SwitchConfirmation {
