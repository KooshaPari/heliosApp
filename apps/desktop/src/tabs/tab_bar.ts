...
export class TabBar {
	...
	selectTab(tabId: string): void {
		...
		if (this.selectedTabId) {
			const prevTab = this.tabs.find(
				(t) => t.getTabId() === this.selectedTabId,
			);
			if (prevTab) {
	...
	private renderTabHeader(tab: TabSurface, index: number): HTMLElement {
		...
		headerEl.setAttribute(
			"tabindex",
			this.focusedTabIndex === index ? "0" : "-1",
		);
		...
		headerEl.style.borderBottom = isSelected
			? "2px solid #1976d2"
	...
	updateTabs(tabs: TabSurface[]): void {
		...
		this.tabOrder = [
			...this.tabOrder.filter((id) => newIds.includes(id)),
			...addedIds,
		];
