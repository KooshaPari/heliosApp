...
function parseCoverageData(): { packages: Map<string, CoverageMetrics>; aggregate: CoverageMetrics } {
  ...
  let coverageJson: Record<string, {
  ...
  if (!coverageJson) {
    console.warn('No coverage-final.json found. Run vitest with --coverage first.');
    return { packages, aggregate };
  }

  // Group files by top-level package (apps/runtime, apps/desktop, packages/*)
  const pkgTotals = new Map<string, { covS: number; totS: number; covF: number; totF: number; covB: number; totB: number }>();
  ...
  for (const [filePath, data] of Object.entries(coverageJson)) {
    ...
    if (!pkgTotals.has(pkgName)) {
      pkgTotals.set(pkgName, { covS: 0, totS: 0, covF: 0, totF: 0, covB: 0, totB: 0 });
    }
  ...
  let totalCovS = 0, totalTotS = 0, totalCovF = 0, totalTotF = 0, totalCovB = 0, totalTotB = 0;

  for (const [pkgName, t] of pkgTotals) {
    const pct = (covered: number, total: number) => total === 0 ? 100 : Math.round((covered / total) * 10000) / 100;
    ...
    totalCovS += t.covS; totalTotS += t.totS;
    totalCovF += t.covF; totalTotF += t.totF;
    totalCovB += t.covB; totalTotB += t.totB;
  }
