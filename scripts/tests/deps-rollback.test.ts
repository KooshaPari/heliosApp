...
describe("Dependency Rollback Integration", () => {
  beforeEach(() => {
    ...
    // Ensure deps-changelog.json exists (may be removed by concurrent tests)
    try {
  ...
  test("rollback target package exists in manifest", () => {
    ...
    expect(registry.dependencies.some(d => d.name === "electrobun")).toBe(true);
  });
  ...
  test("package not in manifest returns error", () => {
    const registry: DepsRegistry = JSON.parse(readFileSync(REGISTRY_PATH, "utf-8"));
    const notFound = registry.dependencies.find(d => d.name === "nonexistent-package-xyz");
  ...
  test("changelog entry can be appended for rollback outcome", () => {
    const changelog: DepsChangelog = JSON.parse(readFileSync(CHANGELOG_PATH, "utf-8"));
    ...
    expect(changelog.entries[changelog.entries.length - 1].outcome).toBe("success");
  });
  ...
  test("concurrent changelog appends produce consistent state", () => {
    ...
    expect(changelog.entries[changelog.entries.length - 2].package).toBe("concurrent-1");
    expect(changelog.entries[changelog.entries.length - 1].package).toBe("concurrent-2");
  });
});
